...
void debug_itr(RandomIt start, RandomIt end, int split = SPLIT) {
  ...
  if (!general_debugging) cerr << ENDL;
}
...
void debug_itr(const T items[][s], int len, int split = s) {
  if (split != s) {
    ...
    for (int i = 0; i < len; i++)
      for (int j = 0; j < split; j++) newitems[i][j] = items[i][j];
...
ostream &operator<<(ostream &os, const queue<T> &x) {
  auto copy = x;
  vector<T> v;
  while (copy.size()) v.push_back(copy.front()), copy.pop();
...
ostream &operator<<(ostream &os, const priority_queue<T> &x) {
  auto copy = x;
  vector<T> v;
  while (copy.size()) v.push_back(copy.top()), copy.pop();
  debug_one(v);
  return os;
}
#endif
...
template <typename T, enable_if_t<is_iterable<T>::value, bool> = true>
...
void debug_one_helper(string t) { cerr << '"' << t << '"'; }
